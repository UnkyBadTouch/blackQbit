    ...
    {Object.entries(categories).map(([name, c]) => (
      <Fragment key={name}>
        <div className="serverrow">
          ...
          <button className="chip" onClick={() => { setEditCat(editCat === name ? null : name); setCatPath(c.savePath || '') }}>Edit</button>
          {confirmDel === 'cat:' + name
        ...
        {editCat === name && (
          <div className="pwrow">
            <input value={catPath} onChange={e => setCatPath(e.target.value)} placeholder={`Save path for "${name}"`} />
